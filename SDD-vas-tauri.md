...
- [ ] 文字浮水印：字型大小、顏色、不透明度變更後 → 輸出圖片反映設定
- [ ] 圖片浮水印：選取 Logo 後轉換 → 輸出圖片含 Logo 疊圖
- [ ] 圖片浮水印：不透明度 50% → Logo 呈半透明
- [ ] 浮水印位置：點選 9-grid 各格 → 轉換結果浮水印位置正確
- [ ] 邊距設為 0 → 浮水印貼邊；設為 100 → 浮水印往內縮 100px
- [ ] 文字 + 圖片同時啟用，各自選擇不同位置 → 兩者同時清晰出現（例如文字 southwest、圖片 southeast）
- [ ] 文字 + 圖片同時啟用，選擇相同位置 → toast 警告「位置相同，請各自選擇不同位置」，轉換被阻擋
- [ ] 無可用系統字型時 → 文字浮水印靜默略過，圖片浮水印仍套用
