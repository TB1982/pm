...
| v1.6 | 2026-03-30 | Sprint 2 規劃完成：DoR 全通過（S2-01～S2-05）；Explore① 完成（PDF/ShareSheet/OCR 方案確認）；§ 3.1 IPC Contract Sprint 2 補入；功能規格與 TDD 寫入 |
...
| `save_image_as` | `image_data: String, format: String` | `Result<String, String>` | 彈出儲存對話框；image_data 為 base64 data URL；format 為 `"png"/"jpg"/"webp"/"pdf"`；回傳儲存路徑；取消回傳空字串 |
| `copy_to_clipboard` | `image_data: String` | `Result<(), String>` | 燒錄整張畫布寫入系統剪貼簿（PNG 格式）；image_data 為 base64 data URL |
| `drag_out` | `image_data: String` | `Result<(), String>` | 寫入暫存 PNG → 觸發 `tauri-plugin-drag` 拖曳；拖曳完成後刪除暫存檔 |
| `share_sheet` | `image_data: String, x: f64, y: f64` | `Result<(), String>` | 燒錄畫布為暫存 PNG → 呼叫 macOS `NSSharingServicePicker`；(x,y) 為觸發按鈕的螢幕座標；分享完成後刪除暫存檔 |
| `get_brand_colors` | — | `Result<Vec<String>, String>` | stub：取得品牌色陣列 |
| `save_brand_colors` | `colors: Vec<String>` | `Result<(), String>` | stub：儲存品牌色 |
| `ocr_recognize` | `image_data: String, region: Option<serde_json::Value>` | `Result<serde_json::Value, String>` | Vision `VNRecognizeTextRequest`；image_data 為 base64 data URL；region 為選取範圍 `{x,y,w,h}`（null = 全圖）；回傳 `{text: String, lines: Vec<{text,rect}>}` |
| `privacy_scan` | `image_data: String, region: Option<serde_json::Value>` | `Result<Vec<serde_json::Value>, String>` | 五層偵測（NSDataDetector + Regex）；region null = 全圖；回傳 `[{x,y,w,h,type}]` 遮蔽座標陣列 |
...
| Sprint 2 | 2026-03-30 – TBD | S2-01 存檔/複製/拖曳匯出/Share Sheet（in-sprint）；S2-02 編輯器視窗 UI（in-sprint）；S2-03 OCR + 隱私遮蔽（in-sprint）；S2-04 Modal 修復（in-sprint）；S2-05 工具預設值 UI bug（in-sprint） | DoR + Explore① 完成 2026-03-30；開始實作 |
...
| **Share Sheet（原生分享）** | ~~Sprint Candidates~~ → **Sprint 2 in-sprint**（S2-01）；`tauri-plugin-sharekit` 呼叫 `NSSharingServicePicker` | Tauri |
...
- [x] 工具列高度約 44px
- [x] 關閉工具列 → 整個 app 結束（從 Dock 強制結束 app 即驗證；工具列本身無關閉按鈕，此為預期設計）

#### 拖動把手
- [x] 拖動把手（⠿）可拖動工具列到螢幕任意位置
- [x] 拖動把手 hover → 發光效果出現（WKWebView non-key window 限制，已移入 Wishlist，本版接受此行為）
- [x] 拖動把手 hover 離開 → 效果消失（同上）

#### Icon 與 Tooltip
- [x] 工具列只顯示 icon，無常駐文字標籤
- [ ] Hover 任一 icon → tooltip 出現（顯示正確工具名稱）　← 後續 Sprint
- [ ] Tooltip 文字對應 i18n（zh / en / ja 切換正確）　← 後續 Sprint

#### 檔案拖入
- [ ] 編輯器未開啟，拖 PNG 進工具列 → 編輯器開啟，圖片為底圖　← 後續 Sprint
- [ ] 編輯器未開啟，拖 JPG 進工具列 → 編輯器開啟，圖片為底圖　← 後續 Sprint
- [ ] 編輯器未開啟，拖 WebP 進工具列 → 編輯器開啟，圖片為底圖　← 後續 Sprint
- [ ] 編輯器已開啟，拖圖進工具列 → 圖片作為疊圖物件插入，不複寫底圖　← 後續 Sprint
- [ ] 拖非圖片檔案進工具列 → 無反應或 toast 提示　← 後續 Sprint

#### 截圖流程
- [x] 點全螢幕截圖 → 工具列隱藏 → 截圖完成 → 工具列恢復 → 編輯器開啟截圖
- [x] 多螢幕時點全螢幕 → 顯示螢幕選擇 picker → 選擇任一螢幕 → 正確截取該螢幕
- [x] 視窗截圖 → picker 顯示視窗縮圖清單 → 選擇視窗 → 工具列隱藏 → 截圖完成 → 工具列恢復原位 → 編輯器開啟截圖
- [x] 視窗截圖後立即關閉編輯器 → 工具列恢復在截圖前的原始位置（不飄移，反覆測試 10+ 次穩定）
- [x] 點矩形截圖 → 工具列隱藏 → 截圖完成 → 工具列恢復 → 編輯器開啟截圖
- [x] 視窗清單不含系統 UI（控制中心、通知中心等）
- ~~編輯器已開啟時截圖 → 截完後複寫編輯器內容~~ — **設計無效**：編輯器開啟時工具列隱藏，無截圖入口。此案例刪除。
...
- [x] 編輯器開啟後，`get_editor_init` 回傳 `mode=blank`、正確 width / height / bg_color（間接驗證：畫布以正確尺寸與背景色渲染即確認）
- [x] 點「開啟」→ 選擇圖片 → 編輯器視窗開啟，圖片載入為底圖
- [x] 編輯器開啟後，`get_editor_init` 回傳 `mode=file`、正確 file_path（間接驗證：圖片正確顯示為底圖即確認）
...
- [x] 取消對話框 → 工具列無反應，不報錯，編輯器視窗不出現

#### 批次轉 — 選擇檔案（select-batch-files）
- [x] 批次轉 modal 中點「選擇檔案」→ 多選對話框彈出
- [x] 選擇 1 張圖片 → 檔案列表顯示該檔案
- [x] 選擇 3 張圖片 → 檔案列表顯示 3 筆
- [x] 取消對話框 → 檔案列表不變
- [x] 再次點「加入更多」→ 新選的檔案附加至既有清單

#### 批次轉 — 上限（select-batch-files）
- [x] 選取超過 100 張圖片 → 只保留前 100 張，其餘截斷

#### 批次轉 — 輸出目錄（select-output-dir）
- [x] 點「選擇目錄」→ 資料夾選擇對話框彈出
- [x] 選擇目錄後 → 路徑顯示在 UI 上（顯示最後一段目錄名）
- [x] 取消 → 路徑欄位不變

#### 批次轉 — 浮水印圖片（select-watermark-image）
- [x] 浮水印啟用時點「選擇圖片」→ 圖片選擇對話框彈出
- [x] 選擇圖片 → 圖片路徑顯示在 UI 上
- [x] 取消 → 路徑欄位不變
...
### S2-01　存檔 / 複製 / 拖曳匯出 / Share Sheet

**Sprint：** 2　｜　**狀態：** `in-sprint`　｜　**DoR 通過：** 2026-03-30

#### 使用者故事
> 身為 VAS 使用者，我想要把標注完的圖片存成檔案、複製到剪貼簿、拖曳匯出，或透過 macOS 原生分享，這樣我才能真正用到標注的成果。

#### 功能規格

| 按鈕 | 行為 | 視窗狀態 |
|------|------|---------|
| 完成並儲存 | 彈出格式對話框（PNG / JPG / WebP / PDF）→ 選路徑 → 輸出合併平面圖 → 關閉編輯器 | 關閉 |
| 複製 | 燒錄整張畫布為 PNG → 寫入系統剪貼簿 → toast「已複製」 | 留著 |
| 拖曳匯出 | 畫布序列化為 PNG → 寫入暫存檔 → `tauri-plugin-drag` 啟動拖曳 → 完成後刪暫存 | 留著 |
| 分享 | 燒錄畫布 → `tauri-plugin-sharekit` 呼叫 `NSSharingServicePicker` → 完成後刪暫存 | 留著 |

- 取消存檔對話框 → 回到編輯視窗，不報錯
- 空白畫布（無標注）→ 四個功能均正常執行
- 失敗（磁碟滿 / 權限不足）→ toast 提示錯誤原因
- PDF：單頁，圖片尺寸等於畫布尺寸，使用 `printpdf` crate

#### 技術依賴
- `printpdf = "0.5"` （PDF 輸出）
- `tauri-plugin-drag = "2.1"` （拖曳匯出）
- `tauri-plugin-sharekit` （Share Sheet）

## 6. 測試案例（TDD）— S2-01

#### 完成並儲存
- [ ] 點「完成並儲存」→ 格式選擇對話框出現，含 PNG / JPG / WebP / PDF 選項
- [ ] 選擇 PNG → 存檔 → 輸出檔案可在 Finder 開啟，內容含底圖 + 所有標注
- [ ] 選擇 JPG → 存檔成功
- [ ] 選擇 WebP → 存檔成功
- [ ] 選擇 PDF → 存檔成功，macOS Preview 可開啟
- [ ] 取消對話框 → 回到編輯視窗，不報錯，編輯器不關閉
- [ ] 存檔成功 → 編輯器視窗關閉 → 工具列恢復原位（KM-008 路徑）
- [ ] 空白畫布存檔 → 輸出純色圖片，不報錯

#### 複製到剪貼簿
- [ ] 點「複製」→ toast「已複製」→ 貼到 Google Docs / 社群軟體顯示正確圖片
- [ ] 複製後編輯器留著，可繼續操作
- [ ] 空白畫布複製 → 貼上為純色圖片，不報錯

#### 拖曳匯出
- [ ] 從「拖曳匯出」按鈕拖出 → 可放入 Finder 資料夾，產生 PNG 檔案
- [ ] 拖曳完成後暫存檔自動刪除
- [ ] 拖曳取消（放回原處）→ 暫存檔仍刪除，不報錯

#### Share Sheet
- [ ] 點「分享」→ macOS Share Sheet 出現，列出可用分享目標
- [ ] 選擇 AirDrop → 分享成功
- [ ] 分享完成後暫存檔自動刪除
- [ ] 取消分享 → 暫存檔刪除，不報錯
...
#### 使用者故事
> 身為 VAS 使用者，我希望編輯器視窗外觀和 Electron 版一致，沒有原生標題列佔空間，這樣畫布區域才完整。

#### 功能規格

- `open_editor_window()` 加入 `.title_bar_style(tauri::TitleBarStyle::Overlay)` + `.hidden_title(true)`
- macOS 紅黃綠燈保留（原生，不拔除）
- 頂部屬性列整排加 `data-tauri-drag-region`，可拖動視窗
- 頂部中央顯示「🍾 VAS」（小瓶子圖標 + 文字）
- 畫布空間恢復原生標題列所佔高度

## 6. 測試案例（TDD）— S2-02

- [ ] 編輯器開啟 → 不顯示原生標題列，不顯示「VAS Editor」文字
- [ ] 頂部顯示紅黃綠燈 + 中央「🍾 VAS」
- [ ] 拖曳頂部屬性列 → 視窗跟著移動
- [ ] 點紅燈 → 編輯器關閉 → 工具列恢復原位（KM-008 路徑）
- [ ] 編輯器畫布高度 > Electron 版修復前（標題列空間還回）
...
#### 使用者故事
> 身為 VAS 使用者，我希望編輯器內的掃描文字與隱私遮蔽功能正常運作，這樣 VAS 的核心智慧功能才能在 Tauri 版繼續使用。

#### OCR 功能規格

- 使用 `objc2-vision` crate，呼叫 `VNRecognizeTextRequest`
- 輸入：畫布選取範圍的 base64 data URL
- 輸出：辨識文字 + 各行 bounding box
- 找不到文字 → toast 提示
- 結果顯示於浮動面板，含「複製文字」按鈕
...
| 層 | 方法 | 實作 |
|----|------|------|
| 1 | 電話 / Email / 實體地址 | `objc2-data-detection` crate（NSDataDetector）|
| 2 | 人名（標籤感知）/ 組織名 | Rust regex：`姓名：(.+)`、`負責人：(.+)` 等；`(.+?)有限公司`、`(.+?)股份有限公司` |
| 3 | 身分證 / 統編 / 信用卡 / IPv4/6 / API Token | Rust regex |
| 4 | 臺灣地址格式 | Rust regex |
| 5 | 中文標籤感知 | Rust regex（`聯絡人：`、`收件人：`、`密碼：` 等） |
...
#### 技術依賴
- `objc2-vision` （OCR）
- `objc2-data-detection` （NSDataDetector Layer 1）
- `regex` crate（Layers 2-5，已在 Cargo.toml）

## 6. 測試案例（TDD）— S2-03

**OCR**
- [ ] 拖曳框選含文字的區域 → 浮動面板顯示辨識出的文字
- [ ] 點「複製文字」→ 文字寫入剪貼簿
- [ ] 框選空白或純圖案區域 → toast 提示找不到文字
- [ ] 中英文混合圖片 → 兩種語言均辨識

**隱私遮蔽**
- [ ] 按 K 全圖掃描含身分證號的圖片 → 身分證號被遮蔽
- [ ] 按 K 全圖掃描含電話號碼的圖片 → 電話被遮蔽
- [ ] 按 K 全圖掃描含 Email 的圖片 → Email 被遮蔽
- [ ] 按 K 全圖掃描含 `姓名：王小明` → 姓名值被遮蔽
- [ ] 按 K 全圖掃描含 IPv4 / API Token → 被遮蔽
- [ ] 按 K 掃描日期 `20260330` → **不**被遮蔽（統編日期排除邏輯）
- [ ] 按 K 掃描一般中文說明文字 → 不被遮蔽
- [ ] 拖曳指定範圍掃描 → 只遮蔽範圍內的敏感資料
- [ ] 全圖掃描無敏感資料 → toast「未偵測到敏感資訊」
- [ ] 遮蔽套用後切選取工具 → 可調整遮蔽範圍大小
- [ ] 遮蔽套用後 ⌘Z → 遮蔽物件消失

---

### S2-04　Modal 修復（一鍵套版 + 調整大小）

**Sprint：** 2　｜　**狀態：** `in-sprint`　｜　**DoR 通過：** 2026-03-30

#### 使用者故事
> 身為 VAS 使用者，我希望一鍵套版和調整大小的設定視窗能正確彈出，這樣我才能套版和改變畫布尺寸。

#### 功能規格

**一鍵套版**
- 所見即所得：調整即時反映在畫布
- 直接關閉 modal 維持最後狀態（無確認按鈕）
- 套版結果可 ⌘Z 還原

**調整大小（新行為）**
- 寬高都可以改（Electron 版只能改寬）
- 改一邊另一邊等比例自動計算
- 最小 100 × 100 px；最大 8192 × 8192 px
- 超出範圍 / 非數字 / 0 → toast 提示

**共同行為**
- ESC 或點取消 → 關閉 modal，畫布不變

## 6. 測試案例（TDD）— S2-04

**一鍵套版**
- [ ] 點「一鍵套版」→ 設定 modal 正確彈出
- [ ] 調整套版參數 → 畫布即時反映變化（所見即所得）
- [ ] 直接關閉 modal → 最後套版狀態保留在畫布上
- [ ] 套版後 ⌘Z → 畫布還原套版前狀態
- [ ] ESC → modal 關閉，畫布不變

**調整大小**
- [ ] 點「調整大小」→ 設定 modal 正確彈出，顯示目前寬高
- [ ] 輸入新寬度 → 高度欄位自動等比例更新
- [ ] 輸入新高度 → 寬度欄位自動等比例更新
- [ ] 輸入 50（低於最小值）→ toast 提示，不套用
- [ ] 輸入 9000（高於最大值）→ toast 提示，不套用
- [ ] 輸入 0 或非數字 → toast 提示，不套用
- [ ] 輸入合法值確認 → 畫布尺寸更新
- [ ] ESC / 取消 → modal 關閉，畫布尺寸不變

---

### S2-05　工具預設值與 UI Bug 修復

**Sprint：** 2　｜　**狀態：** `in-sprint`　｜　**DoR 通過：** 2026-03-30

#### 使用者故事
> 身為 VAS 使用者，我希望每個標注工具打開時有正確的預設值、控制項都能點，這樣我不用每次都手動修設定才能開始用。

#### 修復清單

| # | 工具 | 問題 | 修復方向 |
|---|------|------|---------|
| 1 | 線條 / 筆形 | 粗細預設值為 0，視覺尺寸偏小 | 查 `devicePixelRatio` 初始化邏輯 |
| 2 | 編號 | 預設大小跑掉 | 同上（解析度問題） |
| 3 | 馬賽克 | 模式切換 button 點不到 | 查 CSS pointer-events / z-index |
| 4 | 符號 | dropdown 無法展開；大小選擇無效 | 查 dropdown 事件監聽器 |
| 5 | 矩形框 | 鏡映功能失效 | 查水平/垂直鏡射的 canvas transform |
| 6 | 矩形框 | 工具列超出視窗寬度（1200px 預設）| CSS overflow / flex-wrap 修正 |
| 7 | 色塊 | Shift 無法限制正方形/正圓 | 先查與矩形框的差異，再對齊矩形框行為 |

- 驗收標準：全部對齊 Electron 版行為

## 6. 測試案例（TDD）— S2-05

- [ ] 線條工具：預設粗細顯示視覺尺寸與 Electron 版一致（非 0）
- [ ] 筆形工具：預設粗細顯示視覺尺寸與 Electron 版一致（非 0）
- [ ] 編號工具：新建編號預設大小與 Electron 版一致
- [ ] 馬賽克：點擊「馬賽克」/「模糊」button → 模式切換成功
- [ ] 馬賽克：比例參數（區塊尺寸 / 模糊半徑）可點擊並生效
- [ ] 符號：點擊符號選單 → dropdown 正確展開
- [ ] 符號：選擇符號後大小調整 → 符號大小即時更新
- [ ] 矩形框：選取後點水平鏡射 → 物件水平翻轉
- [ ] 矩形框：選取後點垂直鏡射 → 物件垂直翻轉
- [ ] 矩形框：工具列在 1200px 寬視窗內完整顯示，不溢出
- [ ] 色塊：繪製時按住 Shift → 限制為正方形 / 正圓
