...
v1.6 — 新增批次格式轉換功能（F1-C）：第 6 顆工具列按鈕（🔄 批次轉）；工具列寬度 440→514px；支援 PNG/JPG/WebP/GIF/SVG 來源；轉換目標 PNG/JPG/WebP/GIF；可選品質（JPG/WebP）、SVG 輸出寬度（預設 1920px）、統一調整尺寸（固定寬度或高度，等比例縮放）、輸出位置（同目錄或指定目錄）、轉換後刪除原始檔；同格式偵測：轉換前彈出警示並提供「略過 / 取消全部」；轉換中逐檔顯示進度條與 log
v1.5 — 主視窗改為浮動工具列模式：(1) 主視窗縮小為 440×68px 水平工具列（frame:false, transparent, alwaysOnTop）；(2) 截圖時工具列 hide()，編輯器全部關閉後自動 show()；(3) 位置可拖曳，重啟後記憶位置（userData/toolbar-pos.json）；(4) 視窗選擇 / 說明 modal 開啟時動態 resize 展開，關閉後縮回工具列；(5) 矩形截圖修正：open-overlay 時 hide() 主視窗，cancel-overlay 時 show() 主視窗；全域快捷鍵觸發路徑與按鈕觸發路徑行為一致
v1.4 — Bug 修復：(1) 存檔 WebP 時加入 .withMetadata() 修正 macOS Quick Look 無法生成桌面縮圖的問題；(2) 編輯器存檔成功後延遲 800ms 自動關閉視窗（原本存檔後視窗不會關閉）
