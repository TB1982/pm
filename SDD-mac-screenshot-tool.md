...
| 最近使用色（釘選） | 浮動選色面板標準色彩列下方新增「最近使用」一行（最多 10 色，所有顏色來源混著記，session 結束後重置）；未使用任何顏色前該列隱藏 |
...
> 擴充圖片進入編輯器的入口，不再侷限於「本工具截圖」或「開啟本地檔案」。詳細規格待 v2.7 實作完成後展開。

**功能包：**

| 功能 | 說明 |
|------|------|
| 剪貼簿貼上開啟（Cmd+V） | 工具列偵測 `Cmd+V`；若剪貼簿含圖片則直接開啟編輯器（與「開啟圖片」接同一條路）；若剪貼簿無圖片則不動作 |
| 歷史截圖紀錄 | 自動保留本 session 截取的圖片清單，可快速重新開啟任一張繼續標注 |

**剪貼簿貼上來源情境：**
- 瀏覽器右鍵「複製圖片」→ Cmd+V
- 其他截圖工具截圖後存入剪貼簿 → Cmd+V
- P1-A 設為「關閉」模式下截了圖、事後反悔想標注 → Cmd+V
...
> 多選與複製貼上是同一操作流程的前後兩段，合併為一個版本。依賴前提：v3.3 啟動前，所有標注工具類型（矩形框、色塊、線條、文字、編號、疊圖）均已穩定實作完成。

**功能包：**

| 功能 | 說明 |
|------|------|
| 多選標注（Shift+Click） | `Shift+Click` 可累加選取多個標注元素；選取中的元素同時顯示選取框 |
| 多選拖動 | 所有已選取的標注一起移動，相對位置保持不變 |
| 多選刪除 | `Delete` / `Backspace` 一次刪除所有已選取的標注 |
| 多選換色 | 顏色面板選色後，所有已選取的標注套用同一顏色 |
| 全類型 Cmd+C / Cmd+V | 矩形框、色塊、線條、文字、編號均支援複製貼上（原僅編號支援）；貼出副本位移 +8px |
| 多選複製貼上 | 選取多個標注後 Cmd+C → Cmd+V，整組一起複製，相對位置保持不變 |

**架構影響：**
- `selectedAnnotation`（單一物件）改為 `selectedAnnotations`（陣列），影響選取、移動、刪除、顏色修改的全部邏輯，需完整回歸測試

---

### v3.4 — 標注對齊與分佈

> 依賴 v3.3 多選功能；對標 PowerPoint 的對齊輔助，不追求完整向量編輯器體驗。

**功能包：**

| 功能 | 說明 |
|------|------|
| 對齊左緣 | 所有選取標注的左邊界對齊至最左者 |
| 對齊右緣 | 所有選取標注的右邊界對齊至最右者 |
| 對齊上緣 | 所有選取標注的上邊界對齊至最上者 |
| 對齊下緣 | 所有選取標注的下邊界對齊至最下者 |
| 水平置中 | 所有選取標注的水平中軸對齊 |
| 垂直置中 | 所有選取標注的垂直中軸對齊 |
| 水平等距分佈 | 選取標注依水平方向均勻分佈（間距相等） |
| 垂直等距分佈 | 選取標注依垂直方向均勻分佈（間距相等） |

**UI 觸發：** 多選狀態下，頂部選項列出現對齊按鈕組（8 個圖示按鈕，hover 顯示文字說明）；僅選取單一標注時按鈕組隱藏。
