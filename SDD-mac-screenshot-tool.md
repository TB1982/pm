...
**UI 設計：**
- 按鈕主體顯示 ⏱ 圖示 + 「延遲」標籤
- 按鈕內嵌小型秒數下拉（`<select>`）：選項 **1s / 2s / 3s**
- 點擊下拉僅改變秒數，不觸發截圖
- 點擊按鈕主體（非下拉）觸發倒數

**倒數行為：**
- 觸發後按鈕圖示即時替換為秒數（「3」→「2」→「1」），每秒更新
- 倒數期間可正常操作其他 App（hover 效果、展開選單等）
- 倒數至 0：工具列 `hide()` → 等待 200ms → 全螢幕截圖（同 S1 全螢幕路徑）→ 進入編輯器
- 工具列倒數期間保持可見（不提前 hide），避免截圖內容包含倒數 UI：hide 動作在截圖前最後一刻執行

**取消：**
- 倒數期間按 `Esc` 取消，圖示恢復 ⏱，按鈕恢復可用
