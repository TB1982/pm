...
### v3.41 — 裁切工具修正：handle 翻轉 + 尺寸穩定 + 雙擊確認 + 按鈕動態顯隱 *（2026-03-27）*
...
### v3.40 — 資安修補：XSS 防護 + 畫布尺寸上限 *（2026-03-27）*
...
### v3.37 — QR 閾值調整 + 工具列新開畫布獨立入口 *（2026-03-27）*
...
### v3.34 — 線條曲線控制點（Quadratic Bezier） *（2026-03-27）*
...
### v3.33 — 智慧磁吸對齊（Smart Snap Alignment） *（2026-03-27）*
...
### v3.31 — 複數選取（框選 + Shift 加選） *（2026-03-27）*
...
### v3.30 — 資安升級 + Retina WYSIWYG + 數字工具修正 + contextBridge 後置修補 *（2026-03-27）*
...
### v3.29 — 測試修正批次：歷史按鈕、字型預設、數字大小、裁切復原、疊圖限制、OCR 複製、關閉動畫 *（2026-03-26）*
...
### v3.28 — 隱私遮蔽工具 + 歷史 Drawer 重設計 + Retina DPR 修正 *（2026-03-26）*
...
### v3.27 — 批次 WebP 恢復 + 浮水印 + 去背 + 品牌色庫 + 歷史截圖面板 *（2026-03-25）*
...
### v3.26 — 套版三滑桿 + 批次轉換移除 WebP *（2026-03-25）*
...
### v3.25 — mesh gradient + 社群尺寸（見 v3.24 底部說明） *（2026-03-25）*
...
### v3.24 — 一鍵套版重設計：全改 Apple 漸層六色 *（2026-03-25）*
...
### v3.23 — 一鍵套版 Snapshot 修正（防止層疊套用） *（2026-03-25）*
...
### v3.22 — 一鍵套版（MVP：6 款裝飾框架） *（2026-03-25）*
...
### v3.21 — 符號工具重構：多群組按鈕 + 馬賽克參數顯示修正 *（2026-03-25）*
...
### v3.20 — 符號工具選取框精準化 + 移除重複符號 *（2026-03-25）*
...
### v3.19 — 無縫匯入匯出（MVP：Drag & Drop + 複製 + Drag Out） *（2026-03-25）*
...
### v3.18 — 新工具：馬賽克/模糊 + 符號印章 *（2026-03-25）*
...
### v3.17 — Bug 修正：鉛筆工具粗線箭頭方向跑掉 *（2026-03-25）*
...
### v3.16 — Bug 修正：箭頭露餡 + 缺口 + 調色盤自動收起 *（2026-03-25）*
...
### v3.15 — Bug 修正：鉛筆箭頭與線段斷開 *（2026-03-25）*
...
### v3.14 — Bug 修正：箭頭方形頂點 + 外框位移 *（2026-03-25）*
...
### v3.13 — Bug 修正：箭頭外框金字塔 + 鉛筆箭頭角度偏移 *（2026-03-25）*
...
### v3.12 — Bug 修正：直線端點外框 + 空心圓端點符號 *（2026-03-25）*
...
### v3.11 — Bug 修正：鉛筆端點外框 + 虛實線失效 + 外框群組移至最後 *（2026-03-25）*
...
### v3.10 — Bug 修正 + 端點 UI 精簡 + 外框粗細/虛實線分組 *（2026-03-25）*
...
### v3.9 — Options Bar 全面對齊；框線外框位移；虛實線全工具覆蓋 *（2026-03-24）*
...
### v3.8 — 工具一致性升級（邊框／陰影統一） *（2026-03-24）*
...
### v3.7 — 筆型工具；虛線種類升級；端點樣式升級 *（2026-03-24）*
...
### v3.6 — 框型選取工具（矩形區域複製為浮動圖層） *（2026-03-24）*
...
### v3.5 — 工具列雙排佈局；Smart Semantic Numbering；粗細 px 輸入 *（2026-03-24）*
...
### v3.4 — OCR 改用 macOS Vision；折線縮放；UX 修正 *（2026-03-24）*
...
### v3.1 — OCR 文字辨識工具（Free MVP） *（2026-03-24）*
...
### v3.0 — 橢圓框工具（Shift 拉 = 正圓） *（2026-03-23）*
...
### v2.7 — 編輯器底部三按鈕：複製至剪貼簿 + macOS Share Sheet *（2026-03-23）*
...
### v2.6 — 文字工具 UX 全面升級 + 框線/色塊圓角 + 線條正交折線（字體選擇、字型大小快選、工具列重排、文字對齊、預覽對齊修正、圓角、正交折線） *（2026-03-23）*
...
### v2.5 — 浮動選色面板升級；編號標記大小調整 *（2026-03-23）*
...
### v2.4 — 截圖後開啟編輯器：偏好設定切換 *（2026-03-22）*
...
### v2.3 — 長期遷移規劃：Tauri 版本 + UI 中英切換 *（2026-03-23）*
...
### v2.2 — 框線/色塊工具拆分；色塊漸層；編號複製貼上 *（2026-03-22）*
...
### v2.1 — S2 網頁截圖移除；延遲截圖；迭代規劃 v2.1–v2.4 *（2026-03-22）*
...
### v1.7 — 顏色工具列：滴管選色（E2-F）與 Hex 輸入（E2-G） *（2026-03-22）*
...
### v1.6 — 新增批次格式轉換功能（F1-C） *（2026-03-22）*
...
### v2.4 — 截圖後開啟編輯器：偏好設定切換（已實作） *（2026-03-22）*

> 詳見變更紀錄 v2.4。原規劃於此版的「文字沿路徑排列」因版號重新分配，已移至 v3.2。

---

### v2.5 — 浮動選色面板；編號大小升級（已實作） *（2026-03-23）*
...
### v2.6 — 文字工具 UX 全面升級（已實作） *（2026-03-23）*

> 詳見變更紀錄 v2.6。

---

### v2.9 — 選取範圍工具（S4） *（2026-03-23）*
...
### v2.7 — 編輯器底部三按鈕（複製至剪貼簿 + 分享 + 完成並儲存） *（2026-03-23）*

> 規格詳見變更紀錄 v2.7。實作待進行。

---

### v2.8 — 歷史（複數）圖片 *（2026-03-23）*

> 延續 v2.7 輸出動作的概念，讓截圖工作流程支援多張圖片的歷史記錄與管理。詳細規格待 v2.7 實作完成後展開。

---

### v2.9 — 鉛筆工具（自由塗鴉） *（2026-03-23）*
...
### v3.1 — 對話框（Speech Bubble） *（2026-03-24）*
