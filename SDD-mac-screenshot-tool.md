...
### v2.3 — 色塊工具支援線性漸層填色
- fillrect 新增 `fillMode: 'solid' | 'gradient'` 屬性
- 漸層模式：顏色 A + 顏色 B（含「透明」選項）+ 4 方向（左→右、上→下、左上→右下、左下→右上）
- 點擊色票預覽方塊可開啟系統原生檢色器選任意色（gradient A/B 皆支援）
- 整體透明度可用 `fillOpacity`（0–100%）疊加控制
- options bar grpFillColor 改版：[實色] [漸層] 模式切換；漸層時顯示 A→B 雙色票 + 方向按鈕

### v2.2 — 框線/色塊工具拆分；編號標記複製貼上
- 色塊工具（B 鍵）：新增 `fillrect` annotation 類型，支援填色（含透明度 0–100%）+ 邊框（可開關、可換色、可設粗細）
- options bar 新增「填色」區塊（色票 + 透明度輸入 + 邊框有/無切換），顯示於 grpColor（邊框色）之前
