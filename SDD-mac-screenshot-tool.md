...
**日期：** 2026-03-21
**狀態：** 待審閱
**變更紀錄：** v0.2 — 修正 E1 最小縮放精度、確認分發方式、調整後續迭代清單、新增快捷鍵規格節（K1）
...
> **待確認：** 你是否有慣用的全域快捷鍵偏好？上述為預設建議，可全部自訂。
...
1. **全域快捷鍵組合**：`Cmd+Ctrl+1/2/3` 是否與你其他工具衝突？
2. **矩形選取截圖**：這應該是最常用功能，是否要給它最順手的位置（例如 `Cmd+Ctrl+Space`）？
3. **快捷鍵說明面板**：是否需要在 App 內放 `?` 或 `Cmd+/` 顯示快捷鍵清單？
...
| Q3 | 全域快捷鍵預設值 | ⏳ 待確認（見第 6 節 K1 討論點） |

---

*本文件為 SDD v0.2，請審閱快捷鍵規格（第 6 節）後確認或提出修改。*
