...
**狀態：** 待審閱
**變更紀錄：**
v1.0 — 編輯器新增縮放與平移功能：工具列放大（⊕）/縮小（⊖）/適合視窗（⊟）按鈕；上方工具列下拉選單直選比例（10%–400%）；放大/縮小工具狀態下拖曳 = 手掌平移、點擊 = 以游標為中心縮放；Ctrl+Scroll / Trackpad Pinch 縮放；快捷鍵 ⌘= / ⌘- / ⌘0；縮放範圍 10%–400%；canvas-scroll-content wrapper 修正大圖縮放後的捲動問題
...
### E0 縮放與平移（視圖縮放，不影響輸出）
- [ ] 點擊工具列「⊕」按鈕，畫面放大約 25%（×1.25），底部比例標籤更新
- [ ] 連續點擊「⊕」三次，比例持續累加放大，不超過 400%
- [ ] 點擊工具列「⊖」按鈕，畫面縮小約 20%（÷1.25）
- [ ] 縮放至 10% 後繼續點「⊖」，比例維持在 10%（不低於最小值）
- [ ] 點擊「⊟」按鈕（適合視窗），比例回到初始 fit 比例，捲動歸零
- [ ] 上方下拉選單選取「200%」，畫面立即縮放至 200%
- [ ] 下拉選單只顯示預設值（10%/25%/50%/75%/100%/150%/200%/300%/400%）；比例不在預設值時選單無選中項目，底部標籤仍顯示準確 %
- [ ] 切換至放大工具後，在畫布上拖曳，畫面平移（不標註）；鬆開後不觸發縮放
- [ ] 切換至放大工具後，在畫布上「點擊」（非拖曳），以點擊位置為中心放大
- [ ] 切換至縮小工具後，點擊畫布，以點擊位置為中心縮小
- [ ] 快捷鍵 ⌘= 放大一段，⌘- 縮小一段，⌘0 適合視窗
- [ ] Trackpad Pinch（雙指縮放）以游標為中心縮放
- [ ] Ctrl+滾輪以游標為中心縮放
- [ ] 圖片放大超過視窗後可上下左右捲動，捲動後視圖移動正確
