...
### v2.7 — 新增 S3 分享功能（macOS Share Sheet）

#### 分享功能（新功能）
- 編輯器右下角新增「分享」按鈕（位於「完成並儲存」左側）
...
- 暫存檔命名規則與儲存檔案一致（`YYYY-MM-DD-HH-mm-ss-share.png`），分享完成後自動清理
...
**觸發方式：** 編輯畫面右下角「分享」按鈕（位於「完成並儲存」左側）
...
**UI 細節：**
- 「分享」按鈕樣式：次要按鈕（灰底），不搶佔「完成並儲存」的視覺主序
- 按鈕 icon：`⬆` 或系統分享圖示（SF Symbol：`square.and.arrow.up`）
...
#### 按鈕狀態
- [ ] 編輯器開啟圖片後，「分享」按鈕為可點擊狀態
- [ ] 編輯器未開啟任何圖片時，「分享」按鈕為 disabled（灰色，無法點擊）
- [ ] 「分享」按鈕位於「完成並儲存」按鈕左側，視覺樣式為次要按鈕（不搶佔主序）
...
| 馬賽克 / 模糊工具 | 拖拉選取矩形區域套用馬賽克或高斯模糊，用於遮蓋個資或敏感資訊 | → v2.9 |
| 曲線箭頭（貝茲把手） | 畫完箭頭後，線段中間出現可拖動的控制把手；拖動把手使箭頭彎曲（quadratic Bézier） | → v3.0 |

---

### v2.4 — 截圖後開啟編輯器：偏好設定切換（已實作）

> 詳見變更紀錄 v2.4。原規劃於此版的「文字沿路徑排列」因版號重新分配，已移至 v3.1。
...
### v2.7 — S3 分享功能（macOS Share Sheet）
...
| 功能 | 說明 |
|------|------|
| 水平對齊切換 | 文字工具選項列新增靠左 / 置中 / 靠右三個對齊 icon 按鈕，套用 Canvas `textAlign` 屬性 |
| 對齊狀態反映 | 選取既有文字標註時，對齊按鈕高亮反映其目前對齊方式 |
| 預覽與成品一致 | 即時預覽框與最終繪製結果皆依對齊設定正確錨定基準點 |

---

### v2.9 — 馬賽克 / 模糊工具

> 原列於 v2.3 規劃，因優先順序調整移至本版。
...
> ⚠️ **Experimental** — 原規劃為 v2.4，因版號重新分配移至本版。視 v3.0 完成品質與時程決定是否實作。
...
**評估標準：** v3.0 完成後評估技術可行性，視覺效果可接受則進入 v3.1，否則移至長期 Backlog。
