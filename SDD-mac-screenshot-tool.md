...
### v3.3 — 折線整體縮放 + OCR 工作程序錯誤處理

#### 折線整體縮放（新功能）
- 折線選取後，外框新增 8 個縮放手把（4 角 + 4 邊中點），行為與矩形框一致
- 拖曳角落手把 → 等比例縮放（自由縮放，無鎖定比例）
- 拖曳邊緣手把 → 單軸縮放（鎖定對邊）
- 縮放時所有頂點等比例重新映射至新外框
- 外框手把（圓形）與頂點手把（菱形）共存，視覺上可區分

#### OCR 工作程序錯誤處理（修正）
- 修正：當 utilityProcess 子程序意外退出時（例如 `npm install` 尚未執行），OCR 面板卡在「準備中...」不動
- 修正：新增 `child.on('exit', ...)` 監聽，子程序結束時立即回報失敗訊息，解除卡住狀態
- 修正：`utilityProcess.fork()` 本身失敗時也會捕捉錯誤並回報
...
#### 語言包下載（首次使用）
- [ ] 首次使用（語言包不存在）放開選取後，彈出下載確認對話框
- [ ] 對話框顯示語言包大小說明（繁中 ~18MB + 英文 ~4MB）
- [ ] 點擊「取消」：關閉對話框，清除選取框，返回 OCR 工具待機狀態
- [ ] 點擊「確認下載並辨識」：關閉對話框，開始下載並辨識
- [ ] 語言包下載時 OCR 面板進度條顯示「下載語言包 X%」
- [ ] 下載完成後自動繼續辨識，進度條更新為「辨識中... X%」
- [ ] 二次使用（語言包已存在）不再顯示下載對話框，直接辨識
