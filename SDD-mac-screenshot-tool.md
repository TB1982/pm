...
| 功能 | 說明 |
|------|------|
| 延遲截圖 | 工具列新增「延遲截圖」按鈕；點擊後出現浮動倒數視窗（3 秒），倒數期間可操作任意 UI（含 hover、展開選單），時間到自動觸發全螢幕截圖 → 進入編輯器 |
| 截圖同時複製到剪貼簿 | 截圖完成（任何模式）時，影像同時寫入系統剪貼簿；編輯器打開後剪貼簿內容仍在，可 Cmd+V 貼至其他 App（如 Line）而不需先存檔 |
