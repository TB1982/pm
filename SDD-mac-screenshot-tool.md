...
- [ ] 拖曳單一物件接近另一物件左邊緣 8px 內 → 吸附，1 條垂直輔助線出現
- [ ] 拖曳物件水平中線接近畫布水平中線 8px 內 → 吸附
- [ ] 吸附後繼續拖曳超過 8px → 自動脫離，輔助線消失
- [ ] 按住 Alt 拖曳 → 不磁吸，不顯示輔助線
- [ ] 多選群組拖曳：群組右邊緣接近另一物件左邊緣 → 吸附
- [ ] 放開滑鼠 → 輔助線立即消失
- [ ] X 軸與 Y 軸同時磁吸 → 顯示水平 + 垂直各 1 條
- [ ] 畫布上只有一個標注（無其他物件）→ 仍可吸附至畫布邊緣
- [ ] 均分磁吸 Case 1：物件拖至兩物件中間等距位置 → 4 條輔助線出現
- [ ] 均分磁吸 Case 2：物件拖至兩物件右方等距位置 → 4 條輔助線出現
- [ ] 均分磁吸 Case 3：物件拖至兩物件左方等距位置 → 4 條輔助線出現
- [ ] 畫布上少於 2 個其他標注 → 不觸發均分磁吸
...
**OCR 複製衝突**
- [ ] 先點「複製圖片」，再進行 OCR 掃描，點「複製文字」→ 貼入任意文字編輯器應得到純文字（不是圖片）
- [ ] OCR 複製後再度點「複製圖片」→ 貼入仍為圖片（方向正確）

**存檔關閉動畫**
- [ ] 存檔後 toast 顯示「已儲存：xxx」，約 1.2 秒後視窗以 macOS 原生縮小動畫關閉（不閃白、不突然消失）
- [ ] 視窗關閉後工具列正常重新出現

**歷史按鈕互動修正**
- [ ] 滑鼠移到右側「歷史截圖」tab → 游標顯示手指（pointer），可點擊
- [ ] 點擊歷史 tab → 面板滑入；再次點擊 → 面板滑出
- [ ] 浮動匯出按鈕（float-drag-export）游標顯示 grab 並可互動
- [ ] OCR 面板（開啟後）可正常點擊內部按鈕

**文字工具預設值**
- [ ] 新增文字標注時預設字型大小為 24pt（非 48pt）
- [ ] 快速選單包含 12pt、18pt、24pt 選項

**數字工具大小**
- [ ] 新增數字標注時預設半徑為「標準」（24）
- [ ] 小/標準/大 按鈕對應 18/24/32，圓圈大小目視合理

**裁切復原**
- [ ] 確認裁切後按 Ctrl/Cmd+Z → 圖片與標注完整還原為裁切前狀態
- [ ] 還原後再按 Ctrl+Shift+Z（redo）→ 重新套用裁切
- [ ] 裁切後延伸畫布：延伸結果僅包含裁切後的圖片，不出現被裁掉的區域

**疊圖限制移除**
- [ ] 已有疊圖時再次點擊「匯入疊圖」→ 可正常開啟檔案選取，不顯示錯誤
- [ ] Cmd+V 貼上選取區域後，疊圖按鈕仍可正常使用（不誤判為已有疊圖）
- [ ] 同一畫面可同時存在多個 img 類型標注，各自可拖曳縮放
...
#### TDD v3.27
- [ ] 批次轉換格式選單包含 WebP
- [ ] 勾選「加入浮水印」，文字/圖片區塊展開；取消勾選，區塊收起
- [ ] 文字浮水印：輸入文字後轉換，輸出圖片含對應文字
- [ ] 圖片浮水印：選取 Logo 後轉換，輸出圖片含 Logo 疊圖
- [ ] 浮水印位置：點選 9-grid 各格，轉換結果浮水印位置正確
- [ ] 去背按鈕可點擊（不再 disabled）
...
#### 縮放手把外觀
- [ ] 折線選取後，外框顯示 8 個圓形縮放手把（4 角 + 4 邊中點），與矩形框樣式相同
- [ ] 外框縮放手把（圓形）與頂點手把（菱形）同時顯示，視覺上可區分
- [ ] 游標移到角落手把時顯示 `nwse-resize` 或 `nesw-resize`，移到邊緣手把顯示 `ns-resize` 或 `ew-resize`

#### 縮放互動
- [ ] 拖曳角落手把 → 所有頂點等比例縮放，外框對角保持固定
- [ ] 拖曳上下邊緣手把 → 僅垂直縮放，水平不變
- [ ] 拖曳左右邊緣手把 → 僅水平縮放，垂直不變
- [ ] 縮放放開後，折線形狀更新並記錄至 Undo 歷史
- [ ] 縮放至極小（接近 1px）不會讓頂點坐標混亂
...
#### 頂點手把外觀與點擊
- [ ] 折線被選取後，每個頂點顯示菱形（◆）手把，顏色為白底藍框
- [ ] 菱形手把視覺大小約 8px，明顯大於矩形框的圓形手把
- [ ] 游標移到菱形手把時變為 `crosshair`
- [ ] 點擊菱形手把範圍（約 12px 半徑）可開始拖曳該頂點
- [ ] 拖曳頂點後放開，折線形狀更新並記錄至 Undo 歷史

#### Shift 吸附方向
- [ ] 拖曳折線頂點時按住 Shift，移動 X 位移 > Y 位移 → 水平鎖定（y 固定在拖曳起點的 y 值）
- [ ] 拖曳折線頂點時按住 Shift，移動 Y 位移 > X 位移 → 垂直鎖定（x 固定在拖曳起點的 x 值）
- [ ] Shift 水平／垂直鎖定相互切換正確（以拖曳起點為判斷基準，不受相鄰頂點位置影響）

### S5 OCR 文字辨識工具（v3.1）

#### 工具入口與選取操作
- [ ] 點擊工具列「文」按鈕或按 G 鍵，游標變為十字，options bar 顯示「請拖曳選取辨識區域」
- [ ] 在畫布上拖曳，顯示藍色虛線矩形選框 + 淡藍填色
- [ ] 拖曳中 options bar 顯示即時尺寸（寬 × 高 px）
- [ ] 選取框小於 4×4 px 時放開滑鼠不觸發 OCR
- [ ] 按 Esc 清除選取框，狀態恢復初始提示文字
- [ ] 點擊 options bar「取消」按鈕，切換回矩形框工具
...
- [x] 繁體中文辨識正確（已驗證）
- [x] 辨識速度約 1-2 秒（已驗證）
- [ ] OCR 面板關閉時左上「取消」按鈕隨之消失（選取框清除）
- [ ] 辨識完成後進度條隱藏
- [ ] 辨識完成後，右下角浮動面板出現，顯示辨識文字
- [ ] 辨識結果可在面板內直接編輯
- [ ] 辨識無文字時，面板顯示空白並出現 toast「OCR 未辨識到文字」
- [ ] 辨識失敗時，面板顯示錯誤訊息，toast「OCR 辨識失敗」
- [ ] 點擊「複製」：辨識文字寫入系統剪貼簿，toast「文字已複製到剪貼簿」，面板保持開啟
- [ ] 點擊「複製並關閉」：寫入剪貼簿 + toast + 面板關閉
- [ ] 點擊面板右上角「✕」：關閉面板
- [ ] 面板開啟中可繼續使用 OCR 工具對新區域進行辨識（面板內容更新）
- [ ] 面板開啟中可切換至其他工具，面板保持顯示（不自動關閉）

#### 辨識正確性（啟動後手動驗測）
- [ ] 清晰螢幕截圖中的印刷繁體中文文字辨識率 ≥ 85%
- [ ] 清晰螢幕截圖中的英文字母及數字辨識率 ≥ 95%
- [ ] 中英文混排段落可正常辨識並保留空白
