...
> 從 v2.7 示意圖製作過程中發現的高頻遺漏功能，補入本版。規格待確認後展開。

**功能包（初稿）：**

| 功能 | 說明 |
|------|------|
| 選取範圍 | 拖拉繪製矩形選取框，可移動框位置 |
| 複製 | 將選取區域複製至剪貼簿 |
| 剪下 | 複製選取區域 + 清除原位置 |
| 剪裁 | 裁切至選取範圍（對齊現有 E1-B，統一觸發點） |
| 刪除 | 清除選取區域（填白或透明，待確認） |
| 模糊／馬賽克 | 對選取區域套用模糊或馬賽克效果 |
| 聚光燈 | 暗化選取框外區域，突顯焦點（destructive 或 annotation 層，待確認） |
| 填充 | 以目前工具顏色填充選取區域 |

> ⚠️ 細節規格（刪除填色、聚光燈層類型、選取框移動、與裁切工具關係）待討論確認後補入。
